...
async function verificarDesktops() {
  try {
    console.log('🖥️ Verificando equipamentos DESKTOP...\n');
    ...
    // Buscar projeto DESKTOP
    const desktop = await prisma.projeto.findFirst({
      where: {
        nome: { contains: 'DESKTOP', mode: 'insensitive' }
      }
    });

    console.log(`Projeto: ${desktop.nome} (ID: ${desktop.id})\n`);
