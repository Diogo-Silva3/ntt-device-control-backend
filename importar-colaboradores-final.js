...
async function importarColaboradores() {
  try {
    ...
    console.log('📍 Procurando empresa BIMBO...');
    ...
    console.log(`✅ ${dados.length} colaboradores encontrados\n`);
    ...
    console.log(`✅ ${usuariosExistentes.length} usuários encontrados\n`);
    ...
    for (let idx = 0; idx < dados.length; idx++) {
      ...
      try {
        ...
        const unidadeNome = linha['UNIDADE ']?.toString().trim(); // Nota: coluna tem espaço no final
